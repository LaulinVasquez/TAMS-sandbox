...
const rows = {
  total_hours: [{ i_number: 'I-123', max_assigned_hours: 20, total_hours_worked: 22, difference: 2, status: 'Over' }],
  manual_entries: [{ i_number: 'I-123', percentage_manual: 25, hours_inputted: 2, times_inputted: 1, user_or_mobile_entries: 'User' }],
  pacing_report: [{ i_number: 'I-123', days_under_25_minutes: 2 }],
}
...
test('builds imported weekly metrics without converting unavailable data to zero', () => {
  ...
  assert.equal(record.unsubmittedHoursAvailable, false)
...
test('storage rejects duplicates unless replace is explicit and preserves other weeks', () => {
  ...
  const profilesWithImport = applyTimeStatsImports(profiles, { 12: { records: { 'ta-1': { week: 12, workedHours: 22 } } } })
