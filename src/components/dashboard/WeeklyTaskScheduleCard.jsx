...
const supervisor = { id: 'seda-hancer', role: 'Supervisor' }

export default function WeeklyTaskScheduleCard() {
  ...
  const scope = useMemo(() => ({ supervisorId: supervisor.id, semesterId: semesterConfig.id, week: selectedWeek }), [selectedWeek])
  const [completedByWeek, setCompletedByWeek] = useState(() => {
    try { return { [String(defaultWeek)]: loadTaskCompletions({ supervisorId: supervisor.id, semesterId: semesterConfig.id, week: defaultWeek }) } }
    catch { return { [String(defaultWeek)]: new Set() } }
  })
  const tasks = filterTasksForSupervisor(getTasksForWeek(weeklyTaskSchedule, selectedWeek), supervisor.role)
  ...
  function selectWeek(week) {
    ...
    if (!completedByWeek[String(week)]) {
      try {
        const saved = loadTaskCompletions({ supervisorId: supervisor.id, semesterId: semesterConfig.id, week })
