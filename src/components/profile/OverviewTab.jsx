...
import { statusDetails, summarizeProfile } from '../../utils/profileMetrics'

export default function OverviewTab({ profile }) {
  const [assignments, setAssignments] = useState(profile.assignments)
  const metrics = summarizeProfile(profile.weeklyData)
  ...
  const updateLevel = (course, status) => setAssignments(current => current.map(item => item.course === course ? { ...item, status } : item))
