  ...
  return <main className={`min-h-screen px-8 pb-12 pt-[84px] transition-all ${collapsed ? 'ml-16' : 'ml-56'}`}>
    <div className="mx-auto max-w-[1536px] text-[13px]">
      ...
      <section className={`${card} mt-4`}><div className="flex items-center gap-2"><h2 className="text-xs font-bold uppercase">TA Plan</h2><span className="rounded-full bg-amber-50 px-2 py-1 text-[11px] uppercase text-amber-700">Draft</span></div><p className="my-3 text-sm"><b>{weeklyHours.toFixed(2)}</b> hrs/week <b className="ml-5">{nonRegular.reduce((sum, item) => sum + Number(item.hours || 0), 0)}</b> non-regular hrs/term</p>
        ...
        <label className="block text-[13px] font-medium uppercase">Job description<textarea value={plan.jobDescription} onChange={e => updatePlan('jobDescription', e.target.value)} rows="5" className="mt-2 w-full rounded-md border border-slate-300 p-3 text-[15px] font-normal leading-6 normal-case outline-none focus:border-sky-500"/></label>
        <label className="mt-4 block text-[13px] font-medium uppercase">Job duties<textarea value={plan.jobDuties} onChange={e => updatePlan('jobDuties', e.target.value)} rows="3" placeholder="IRA fills in with CL" className="mt-2 w-full rounded-md border border-slate-300 p-3 text-[15px] font-normal leading-6 normal-case outline-none focus:border-sky-500"/></label>
        <label className="mt-4 block text-[13px] font-medium uppercase">Job expectations<textarea value={plan.jobExpectations} onChange={e => updatePlan('jobExpectations', e.target.value)} rows="7" className="mt-2 w-full rounded-md border border-slate-300 p-3 text-[15px] font-normal leading-6 normal-case outline-none focus:border-sky-500"/></label>
        <label className="mt-4 block text-[13px] font-medium uppercase">Position requirements<textarea value={plan.positionRequirements} onChange={e => updatePlan('positionRequirements', e.target.value)} rows="3" placeholder="IRA fills in with CL" className="mt-2 w-full rounded-md border border-slate-300 p-3 text-[15px] font-normal leading-6 normal-case outline-none focus:border-sky-500"/></label>
        <fieldset className="mt-4 border-0 p-0"><legend className="text-[13px] font-medium uppercase">Supplemental training and/or required certificate</legend><label className="mt-2 block max-w-xs text-sm text-slate-700">Requires certification<select value={plan.requiresCertification} onChange={e => updatePlan('requiresCertification', e.target.value)} className="mt-1 block h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-[15px] outline-none focus:border-sky-500"><option>No</option><option>Yes</option></select></label>{plan.requiresCertification === 'Yes' && <label className="mt-4 block text-[13px] font-medium uppercase">Certification / training description<textarea value={plan.certificationDescription} onChange={e => updatePlan('certificationDescription', e.target.value)} rows="3" className="mt-2 w-full rounded-md border border-slate-300 p-3 text-[15px] font-normal leading-6 normal-case outline-none focus:border-sky-500" placeholder="IRA fills in with CL"/></label>}</fieldset>
