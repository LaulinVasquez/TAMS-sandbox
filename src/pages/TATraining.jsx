  ...
  return <main className={`min-h-screen px-[31px] pb-16 pt-[130px] transition-all ${collapsed ? 'ml-16' : 'ml-56'}`}>
    <div className="mx-auto max-w-[1291px]">
      ...
      <Card className="overflow-hidden">
        <header className="p-6 pb-0"><h2 className="font-semibold">Required training</h2><p className="mt-1 text-xs text-muted">Watch each video and confirm your completion below it.</p>
        ...
        {activeTab === 'general-notes' && <section className="p-6 pt-5" aria-labelledby="general-notes-heading">
          <div className="mb-4"><h3 id="general-notes-heading" className="font-semibold text-slate-900">General Teaching Notes and TA Notes</h3><p className="mt-1 text-xs text-muted">Review expectations for documenting general teaching notes and TA-specific notes.</p></div>
          <ResponsiveKalturaPlayer
            width={480}
            height={270}
...
function ResponsiveKalturaPlayer({ src, title, width = 608, height = 402 }) {
  ...
  return <div ref={containerRef} className="mx-auto w-full overflow-hidden rounded-lg border border-slate-200 bg-black shadow-sm" style={{ maxWidth: `${width}px`, height: `${height * scale}px` }}>
    <iframe
      width={width}
      height={height}
...
function VideoCompletionQuestion({ watched, onChange }) {
  return <fieldset className="mx-auto mt-5 max-w-[608px] rounded-lg border border-slate-200 bg-slate-50 p-4">
    ...
    <p className={`mt-3 text-xs font-medium ${watched ? 'text-emerald-600' : 'text-slate-500'}`} role="status">{watched === true ? 'Completed · Video marked as watched' : watched === false ? 'Not completed · Return after watching the video' : 'No answer selected'}</p>
  </fieldset>
}
...
function CompletionChoice({ tone, selected, onClick, children }) {
  ...
  return <button type="button" role="radio" aria-checked={selected} onClick={onClick} className={`inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-semibold transition-colors ${colors}`}>{selected ? <Check size={16} strokeWidth={3} /> : <Circle size={16} />}{children}</button>
}
